...
for i, id_obj in enumerate(lista_ids):
    ...
    if r_circ is not None:
        ...
        if len(y_circ) > 0:
            ax.plot(bc_circ, y_circ, 'o-', color='darkcyan', markersize=3, linewidth=1, label='Circular')
        if len(y_el_des) > 0:
            ax.plot(bc_el_des, y_el_des, 's-', color='indigo', markersize=3, linewidth=1, label='Elíptico')
